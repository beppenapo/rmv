...
$header = (!$r) ? 'Non sono presenti località nel database ' : 'Lista completa delle località presenti nel database ('.$r.')';
?>
<!DOCTYPE html>
<html>
<head>
        ...
        <div id="toggleDiv">
          <div id="newRec">
            <form name="localitaForm" action="#">
              ...
              <input type="search" id="localita" name="localita" placeholder="Inserisci almen 4 caratteri.." required oninvalid="this.setCustomValidity('Il campo non può essere vuoto')"
              oninput="setCustomValidity('')" disabled />
              <input type="submit" class="submit" name="addLoc" value="Aggiungi località" disabled>
              <div id="msg"></div>
            </form>
          </div>
        </div>
  ...
  $('.clear-filter').click(function (e) {
   ...
   $( ".comuneToken" ).autocomplete({
      ...
      select: function( event, ui ) {
        ...
        $("#"+c).val(ui.item.id);
        $("input[name=localita]").prop('disabled', false).on("keyup", function(){
          var i = $(this).val().length;
          i > 3 ? $("input[name=addLoc]").prop('disabled', false) : $("input[name=addLoc]").prop('disabled', true);
        });
      }
    });
    ...
    $('#comune').on('click', function(){
      $('#comune').on('search keyup', function(){
        if(!this.value){
          $('input[name=localita]').val('').prop("disabled", true);
          $('input[name=addLoc]').prop("disabled", true);
        }
      });
